...
namespace RealActiveRadiator
{
    public class ModuleRealActiveRadiator : ModuleActiveRadiator
    {
        ...
        protected double baseElectricRate;
        ...
        [KSPField] public double maxCryoElectricCost = 10;

        [KSPField] public double maxCryoEnergyTransfer = 500.0;
        ...
        {
            base.OnStart(state);
            Dfld_RefrCost = base.Fields["D_RefrCost"];
        }
        ...
        public void FixedUpdate()
        {
            ...
            if (Dfld_RefrCost != null)
                Dfld_RefrCost.guiActive = PhysicsGlobals.ThermalDataDisplay;
        }
        ...
        public override string GetInfo()
        {
            ...
            while (count-- > 0)
            {
                ...
                var num3 = (double) (dragCube.Area[0] + dragCube.Area[1] + dragCube.Area[2] + dragCube.Area[3] +
                                     dragCube.Area[4] + dragCube.Area[5]);
            ...
            if (num2 != 0.0)
            {
                ...
                num4 *= num4;
                num4 *= num4;
                stringBuilder.Append(Localizer.Format("#autoLOC_232057", new[]
                {
                    (num2 * base.part.emissiveConstant * num4 * PhysicsGlobals.StefanBoltzmanConstant * 0.001)
                    .ToString("F0")
                }));
            }
            ...
            stringBuilder.Append(Localizer.Format("#autoLOC_232060", new[]
            {
            ...
            if (this.parentCoolingOnly) stringBuilder.Append(cacheAutoLOC_232071);

            if (maxCryoElectricCost > 0)
            {
                stringBuilder.Append("\n<b><color=#99ff00ff>Max Cryogenic Cooling</color></b>\n");
                stringBuilder.Append(" <b><color=#00C3FFff>@20K = " +
                                     (CoolingEfficiency(20, 300) * maxCryoElectricCost).ToString("F2") + "kW" +
                                     "</color>\n");
                stringBuilder.Append(" <b><color=#00C3FFff>@90K = " +
                                     (CoolingEfficiency(90, 300) * maxCryoElectricCost).ToString("F2") + "kW" +
                                     "</color>\n");
            }
            else
            {
        ...
        protected override void CheckPartCaches()
        {
            ...
            while (count-- > 0)
            {
                ...
                if (radPart is ModuleRealActiveRadiator &&
                    !((ModuleRealActiveRadiator) radPart).cooledByOtherRadiators)
                    this.activeRadiatorParts.Add(part);
                else
                {
                    this.nonRadiatorParts.Add(part);
                    if (part.Modules.Contains("ModuleFuelTanks")) compensatedParts.Add(part);
                }
            }
        ...
        {
            return Math.Max(coolingTemp / (hotTemp - coolingTemp) * cryoCoolerEfficiency.Evaluate((float) coolingTemp),
                0);
        }
        ...
        protected override void InternalCooling(RadiatorData thisRadiator, int radCount)
        {
            if (!base.vessel.IsFirstFrame())
            {
                ...
                if (this.Dfld_RadCount.guiActive) this.D_RadCount = radCount.ToString();
                ...
                while (count-- > 0)
                {
                    ...
                    if (part.temperature > part.maxTemp * part.radiatorMax) this.hotParts.Add(part);
                }
                ...
                for (var i = 0; i < count2; i++)
                {
                    ...
                    var flag = true;
                    if (this.parentCoolingOnly) flag = this.IsSibling(part2);
                    if (flag)
                    {
                        ...
                        if (energyDifference > 0.0) this.coolParts.Add(thermalData);
                    }
                }
                ...
                for (var i = 0; i < cooledParts; i++)
                    if (coolParts[i].Part.temperature < base.part.skinTemperature)
                    {
                        ...
                        var _maxCryoEnergyTransfer = maxCryoElectricCost * coolingEfficiency;
                        double excessHeat = partThermalData.Energy - partThermalData.MaxEnergy;
                        excessHeat /= radCount + cooledParts;
                        var val = Math.Min(Math.Max(0, thisRadiator.EnergyCap - thisRadiator.Energy),
                            _maxCryoEnergyTransfer);
                        var liftedHeatFlux = Math.Min(val, excessHeat) * Math.Min(1.0, cryoEnergyTransferScale) *
                                             refrigerationThrottle;

                        refrigerationCost += Math.Min(maxCryoElectricCost, liftedHeatFlux / coolingEfficiency);
                    }

                for (var i = 0; i < compensatedParts.Count; i++)
                {
                ...
                if (electricResourceIndex >= 0)
                {
                    ...
                    this.part.GetConnectedResourceTotals(ECID, PartResourceLibrary.GetDefaultFlowMode(ECID),
                        out ECAmount, out ECMaxAmount, true);
                    // if the craft has mixed radiator types then availability calculation may be wrong
                    var powerAvailability = ECAmount / (refrigerationCost * radCount + 0.1);
                    if (powerAvailability < 1.0)
                    {
                        ...
                        this.resHandler.inputResources[electricResourceIndex].rate =
                            baseElectricRate + refrigerationCost;
                    }
                    ...
                    if (Dfld_RefrCost.guiActive)
                        D_RefrCost = refrigerationCost.ToString("F4") + " (throttle = " +
                                     refrigerationThrottle.ToString("P0") + ")";
                }
                ...
                for (var j = 0; j < cooledParts; j++)
                {
                    ...
                    excessHeat /= radCount + cooledParts;
                    var _maxEnergyTransfer = radiatorData.Part.temperature >= base.part.skinTemperature
                        ? this.maxEnergyTransfer
                        : _maxCryoEnergyTransfer;
                    var val = Math.Min(Math.Max(0, thisRadiator.EnergyCap - thisRadiator.Energy), _maxEnergyTransfer);
                    var liftedHeatFlux = Math.Min(val, excessHeat);
                    if (this.Dfld_XferBase.guiActive) this.D_XferBase = liftedHeatFlux.ToString();
                    ...
                    if (this.Dfld_Excess.guiActive) this.D_Excess = excessHeat.ToString();
                    if (this.Dfld_HeadRoom.guiActive) this.D_HeadRoom = val.ToString();
                    if (this.Dfld_XferFin.guiActive) this.D_XferFin = liftedHeatFlux.ToString();
                }
                ...
                for (var i = 0; i < compensatedParts.Count; i++)
                {
                    ...
                    if (!(part.temperature < Math.Round(part.maxTemp * part.radiatorMax * 0.99, 4)))
                    {
                        ...
                        {
                            var compensatedFlux =
                                Math.Min(conductionFlux,
                                    CoolingEfficiency(part.temperature, this.part.skinTemperature) *
                                    maxCryoElectricCost) * refrigerationThrottle / radCount;
